// Close Position via WebSocket - MARKET Order with Reduce-Only
...
async fn main() -> Result<()> {
    println!("\n{}", "═".repeat(80));
    println!("🎯 Close Position via WebSocket (MARKET Reduce-Only)");
    println!("    SDK SignerClient + WebSocket Order Submission");
    println!("{}\n", "═".repeat(80));

    // Initialize context using common helper
    println!("📝 Initializing client...");
    let ctx = ExampleContext::initialise(Some("close_position_ws")).await?;
    ...
    // Calculate slippage price (5% tolerance)
    let slippage_pct = 0.05;
    let slippage_price = if is_ask {
        best_bid * (1.0 - slippage_pct) // SELL: 5% below bid
    } else {
        best_ask * (1.0 + slippage_pct) // BUY: 5% above ask
    };

    println!(
        "   Slippage price: ${:.2} ({}% tolerance)\n",
        slippage_price,
        slippage_pct * 100.0
    );

    // ============================================================================
    // STEP 3: Sign Order using SDK SignerClient
    // ============================================================================
    println!("{}", "═".repeat(80));
    ...
    println!("✍️  Signing MARKET reduce-only order...");

    let size_int = (position_size * 10000.0) as i64; // ETH: 4 decimals
    let price_int = (slippage_price * 100.0) as i32; // ETH: 2 decimals (note: i32 for SDK)

    let client_order_index = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)?
        .as_millis() as i64;

    // Use SDK's order types
    ...
    println!("   Size: {} ETH", position_size);
    println!("   Slippage price: ${:.2}", slippage_price);
    ...
    println!("{}", "═".repeat(80));
    println!("✅ WebSocket close test complete!");
