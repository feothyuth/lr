...
impl FillBreaker {
    ...
    fn should_quote(&self, side: QuoteSide, now: Instant) -> bool {
        match side {
            QuoteSide::Bid => self.bid_paused_until.map_or(true, |until| now >= until),
            QuoteSide::Ask => self.ask_paused_until.map_or(true, |until| now >= until),
        }
    }
...
impl QuoteCache {
    ...
    fn record_bid(&mut self, desired: &Option<DesiredQuote>) {
        self.last_bid = desired.as_ref().map(|d| (d.price_ticks, d.base_qty_units));
    }

    fn record_ask(&mut self, desired: &Option<DesiredQuote>) {
        self.last_ask = desired.as_ref().map(|d| (d.price_ticks, d.base_qty_units));
    }
...
async fn main() -> Result<()> {
    ...
    let min_clip = safe_parse(&details.min_base_amount).unwrap_or(1.0 / size_multiplier as f64);
    ...
    if simulate_fills {
        log_action(
            "🎮  FILL SIMULATION ENABLED — will simulate inventory accumulation for testing",
        );
    }
...
fn process_orders_payload(
    ...
    for order_val in orders {
        ...
        let is_ask_raw = order_val.get("is_ask").and_then(|v| v.as_bool());

        // DEBUG: Log the raw is_ask value to understand the API
        if order_index % 10 == 0 {
            // Sample logging to avoid spam
            eprintln!(
                "🔍 ORDER DEBUG: order_index={} is_ask={:?} price={:?}",
                order_index,
                is_ask_raw,
...
fn sync_inventory_from_positions(
    ...
    let matched = positions_obj.get(&MARKET_ID.to_string()).or_else(|| {
